...
fn cursor_walks_through_comment_blocks() {
    ...
    update(&mut state, AppEvent::Action(AppAction::MoveCursor(1)));
    assert_eq!(state.display_cursor, 14);
...
fn context_numbering(state: &AppState) -> Vec<(Option<u32>, u32)> {
    state
        .display_rows
        .iter()
        .filter_map(|row| match row {
...
fn an_expanded_gap_after_an_insertion_trails_the_old_side_behind_the_new() {
    ...
    assert_eq!(
        context_numbering(&state),
        vec![(Some(2), 4), (Some(3), 5), (Some(4), 6)],
        "the first hunk inserted two lines, so the old side trails the new by two"
    );
}
...
fn an_old_line_computed_below_one_is_left_blank_instead_of_wrapping() {
    ...
    let numbering = context_numbering(&state);
    assert_eq!(
        numbering.first().copied(),
        Some((None, 3)),
        "an old line that lands below 1 does not exist, so the cell stays empty"
    );
    assert!(
        numbering
            .iter()
            .all(|(old, new)| old.is_none_or(|line| line >= 1 && line <= *new)),
        "no old number may exceed its new number: {numbering:?}"
    );
}
...
fn jump_to_display_row_lands_directly_on_a_diff_row() {
    ...
    update(
        &mut state,
        AppEvent::Action(AppAction::JumpToDisplayRow(13)),
    );
